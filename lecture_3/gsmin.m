...
while(1) % for each iteration
  ...
  if f(x1) < f(x2) % this is the only difference from the gsmax
    % then x1 should be the estimated minimum
    % then eliminate the region on the left [xl, x2]
    % reset xl_new to x2, xu remains the same
    if (verbose == 1)
      printf("f(x1) < f(x2) -> xopt = x1, xl = x2\n");
    endif
    xopt = x1;
    xl = x2;
  else
    % then x2 should be the estimated minimum
    % then eliminate the region on the right [x1, xu]
    % reset xu_new to x1, xl remains the same
    if (verbose == 1)
      printf("f(x1) > f(x2) -> xopt = x2, xu = x1\n");
    endif
    xopt = x2;
    xu = x1;
  endif
